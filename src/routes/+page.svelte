<script lang="ts">
	...
	const { visible: atelierVisible } = useIntersectionVisibility(
		() => atelierSection,
		{ threshold: 0.4 },
	);
	const { visible: projectsVisible } = useIntersectionVisibility(
		() => projectsSection,
		{ threshold: 0.35 },
	);
	const { visible: contactVisible } = useIntersectionVisibility(
		() => contactSection,
		{ threshold: 0.3 },
	);
...
<main class="relative overflow-hidden bg-paper text-ink">
	...
	{#if easterEggActivated}
		<div
			class="fixed top-24 right-6 z-50 manga-panel border-accent bg-paper p-6 animate-bounce-in max-w-sm"
		>
			<div class="flex items-start gap-3">
				<span class="text-4xl">🎮</span>
				<div class="space-y-2">
					<p class="font-display text-xl uppercase tracking-[0.2em]">
						{t("misc.konami_title")}
					</p>
					<p class="font-mono text-xs text-ink/80">
						{t("misc.konami_text")}
					</p>
				</div>
			</div>
		...
		<div class="relative flex-1 w-full flex items-center">
			...
			<div class="relative z-10 container mx-auto max-w-6xl px-6 py-24">
				...
				>
					...
					<div
						class="flex flex-wrap gap-8 border-y-2 border-ink py-6 font-mono uppercase tracking-[0.4em] text-sm"
					>
						<div
			...
			<div class="marquee" style="--marquee-duration: 22s">
				<div class="marquee__inner">
					{#each [0, 1] as loopIndex}
						<div
							class="marquee__group font-mono uppercase tracking-[0.5em]"
						...
						>
							{#each tickerItems as label}
								<span class="flex items-center gap-4">
									{label}
									<span
										class="inline-block h-2 w-2 rounded-full bg-paper"
									></span>
								</span>
							{/each}
						</div>
	...
	<section id="atelier" class="relative py-28" bind:this={atelierSection}>
		...
		<div
			class="absolute inset-x-0 top-12 mx-auto w-11/12 h-full border-2 border-dashed border-ink pointer-events-none"
		></div>
		<div class="container relative z-10 mx-auto max-w-7xl px-6">
				...
				<div class="mb-12 space-y-8">
					...
					>
						...
						<span class="block text-ink/40"
							>{t("about.title_highlight")}</span
						>
					</h2>
					<div
						class="space-y-4 font-mono text-base leading-relaxed text-ink/80"
					>
						<p
			...
			>
				<div
					class="flex flex-wrap items-center justify-between gap-3 font-mono text-xs uppercase tracking-[0.35em]"
				>
					<span>{t("about.skills.title")}</span>
					<span class="text-[0.6rem] text-ink/60"
						>{t("about.skills.subtitle")}</span
					>
				</div>
				<div class="grid gap-4 md:grid-cols-3 lg:grid-cols-5">
					{#each t("about.skills.groups") as skillGroup, groupIndex}
						<div
							...
							class:opacity-0={!$atelierVisible}
							style="transition-delay: {600 +
								groupIndex * 100}ms; {!$atelierVisible
								? 'clip-path: inset(0 0 100% 0);'
								: 'clip-path: inset(0 0 0% 0);'}"
						>
							<div
								class="flex items-center gap-2 font-mono text-xs uppercase tracking-[0.3em] text-ink/70"
							>
								<span class="inline-block h-2 w-2 bg-accent"
								></span>
								{skillGroup.name}
							</div>
							<div class="flex flex-wrap gap-2">
								{#each skillGroup.items as skill}
									<span
										class="px-3 py-1.5 border border-ink bg-paper text-xs font-mono uppercase tracking-[0.25em] hover:bg-ink hover:text-paper transition-colors"
									>
										{skill}
									</span>
								{/each}
			...
			{#if SHOW_JOURNAL}
				<div class="mt-16 pt-10 border-t-2 border-dashed border-ink">
					...
					<div class="relative max-w-4xl mx-auto">
						<div
							class="flex flex-wrap items-center justify-between gap-3 mb-6 font-mono text-xs uppercase tracking-[0.35em]"
						>
							<span>{t("about.atelier.journal.title")}</span>
							<span class="text-[0.6rem] text-ink/60"
								>{t("about.atelier.journal.subtitle")}</span
							>
						</div>

						<!-- Notebook page -->
						<div
							class="relative bg-paper border-2 border-ink shadow-lg"
						>
							<!-- Hole punches on the left -->
							<div
								class="absolute left-0 top-0 bottom-0 w-12 flex flex-col justify-around items-center py-8"
							>
								<div
									class="w-6 h-6 rounded-full border-2 border-ink bg-paper shadow-inner"
								></div>
								<div
									class="w-6 h-6 rounded-full border-2 border-ink bg-paper shadow-inner"
								></div>
							...
							<!-- Red margin line -->
							<div
								class="absolute left-16 top-0 bottom-0 w-0.5 bg-accent/30"
							></div>
							...
							<div class="pl-20 pr-8 py-8 space-y-0">
								{#each t("about.atelier.journal.entries") as entry, index}
									<div
										...
										class:opacity-0={!$atelierVisible}
										style="transition-delay: {800 +
											index * 100}ms; {!$atelierVisible
											? 'transform: translateX(-20px);'
											: ''}"
									>
										<!-- Ruled line effect -->
										<div
											class="absolute inset-x-0 bottom-0 h-px bg-gradient-to-r from-transparent via-ink/10 to-transparent"
										></div>

										<div class="flex items-start gap-4">
											<!-- Entry number in circle -->
											<div
												class="flex-shrink-0 w-8 h-8 rounded-full border-2 border-ink bg-paper flex items-center justify-center font-display text-sm"
											>
												{index + 1}
											</div>
											...
											<div class="flex-1 space-y-2">
												<div
													class="flex flex-wrap items-center gap-3 text-xs font-mono"
												>
													<span
														class="px-2 py-1 bg-ink text-paper uppercase tracking-[0.2em]"
														>{entry.date}</span
													>
													<span
														class="px-2 py-1 border border-ink uppercase tracking-[0.2em]"
														>{entry.tag}</span
													>
												</div>
												<p
													class="font-mono text-base font-medium leading-relaxed"
												>
													{entry.label}
												</p>
												<p
													class="font-mono text-sm text-ink/70 leading-relaxed"
												>
													{entry.detail}
												</p>
											</div>
							...
							<!-- Bottom note -->
							<div
								class="px-8 py-4 border-t-2 border-dashed border-ink bg-paper/50"
							>
								<p
									class="text-right text-[0.65rem] font-mono uppercase tracking-[0.3em] text-ink/60"
								>
	...
	<TimelineSection {t} sectionId="timeline" />

	<section id="projects" class="relative py-32" bind:this={projectsSection}>
		<div
			class="absolute inset-x-6 top-12 bottom-12 border-2 border-ink/70 pointer-events-none"
		></div>
			...
			<div class="text-center space-y-4">
				...
				>
					...
					<span class="text-accent">
						{t("projects.title_highlight")}</span
					>
				</h2>
				<p
			...
			<div class="space-y-8">
				...
				>
					...
					<div class="text-center">
						<h3
							class="text-2xl font-display uppercase tracking-[0.3em]"
						>
							{t("projects.featured_section.title")}
						</h3>
						<p
							class="mt-1 text-xs font-mono uppercase tracking-[0.4em] text-ink/60"
						>
							{t("projects.featured_section.subtitle")}
						</p>
					</div>
					<div class="h-px flex-1 bg-ink/30"></div>
				</div>
			...
			<div class="space-y-6">
				...
				>
					...
					<div class="text-center">
						<h3
							class="text-2xl font-display uppercase tracking-[0.3em]"
						>
							{t("projects.archive_section.title")}
						</h3>
						<p
							class="mt-1 text-xs font-mono uppercase tracking-[0.4em] text-ink/60"
						>
							{t("projects.archive_section.subtitle")}
						</p>
					</div>
					<div class="h-px flex-1 bg-ink/30"></div>
				</div>
				...
				>
					...
					<button
						...
						class="px-4 py-2 border-2 font-mono text-xs uppercase tracking-[0.3em] transition-all hover:border-ink {activeFilter ===
						'all'
				...
				>
					...
					{#if filteredArchive.length === 0}
						<div class="p-12 text-center">
							<p
								class="font-mono text-sm text-ink/60 uppercase tracking-[0.3em]"
							>
								{t("misc.empty_list")}
							</p>
						</div>
	...
	<section
		id="contact"
		class="relative py-32 bg-ink text-paper"
		bind:this={contactSection}
	>
		<div
			class="absolute inset-0 opacity-10 bg-paper-fiber mix-blend-screen pointer-events-none"
		></div>
		<div class="container relative z-10 mx-auto max-w-4xl px-6">
			<div class="text-center space-y-6 mb-16">
				...
				>
					{t("contact.title")}
					<span class="text-accent">
						{t("contact.title_highlight")}</span
					>
				</h2>
				<p
			...
			<div class="grid gap-8 md:grid-cols-2 mb-12">
				...
				>
					<h3
						class="text-2xl font-display uppercase tracking-[0.2em]"
					>
					...
					<div class="space-y-4 font-mono text-sm text-paper/80">
						<div class="flex items-center gap-3">
							<span
								class="inline-block h-3 w-3 rounded-full bg-accent animate-pulse"
							></span>
				...
				>
					...
					<div class="space-y-4 font-mono text-sm text-paper/80">
						<div class="flex items-start gap-3">
							...
							<div>
								<p
									class="text-xs uppercase tracking-[0.3em] text-paper/60"
								>
