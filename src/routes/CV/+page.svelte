<script lang="ts">
	import cvPdf from "$lib/assets/CV_JULES_RENAUD--GRANGE.pdf";
		...
		>
			<div class="space-y-6">
				...
				<div class="flex flex-wrap gap-4">
					...
					<a
						href={cvPdf}
						download="CV_JULES_RENAUD--GRANGE.pdf"
				...
				<p
					class="text-xs font-mono uppercase tracking-[0.35em] text-ink/60"
				>
			...
			<div
				class="manga-panel w-1/3 border-2 border-ink bg-paper p-6 font-mono text-sm leading-relaxed text-ink/80 space-y-2"
			>
				<p
					class="font-mono text-xs uppercase tracking-[0.4em] text-ink/60"
				>
					{cvContent.contact.title}
				</p>
	...
	<section class="py-12 border-b border-dashed border-ink bg-paper">
		<div class="mx-auto max-w-5xl px-6">
			<div class="manga-panel overflow-hidden border-2 border-ink">
				<div
					class="border-b border-dashed border-ink/50 bg-ink text-paper px-6 py-3 font-mono text-xs uppercase tracking-[0.4em]"
				>
					{cvContent.viewer.banner}
				</div>
				<div
					class="relative h-[calc(100vh-100px)] max-h-[900px] bg-paper"
				>
					<iframe
						src={`${cvPdf}#view=fitH`}
						class="h-full w-full"
					...
					></iframe>
					<div
						class="pointer-events-none absolute inset-4 border-2 border-dashed border-ink"
					></div>
					<p
						class="pointer-events-none absolute left-4 top-4 bg-paper px-3 py-1 font-mono text-[0.65rem] uppercase tracking-[0.4em]"
					>
